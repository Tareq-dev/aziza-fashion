...
function ShippingDetailsPreview({ cart }) {
  ...
  return (
    <div className="md:m-20  md:px-10 px-2 py-14 bg-white">
      <h1 className="md:text-3xl text-xl font-bold pb-8 text-center">
        Preview All Details
      </h1>
      <table className="w-full border py-8">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td className="border w-3/4 h-14 px-6">
              {order?.bkashPaymentNo ||
                order?.nagadPaymentNo ||
                order?.rocketPaymentNo}{" "}
            </td>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td className="border w-3/4 h-14 px-6">{order?.itemsPrice} Tk</td>
          </tr>
          {cart.map((c) => (
            <tr c={c} key={c._id}>
              <td className="border w-1/4  h-14 text-center">{c.name}</td>
              <td className="border w-3/4 h-14 px-6">{c.quantity} Pcs</td>
            </tr>
          ))}
        </tbody>
      </table>
      <div>
        ...
        <div className="flex justify-center items-center py-4">
          <button
            disabled={!check}
            onClick={!check ? undefined : handlePayment}
            className="bg-pink-600 px-4 py-2 font-semibold  text-white rounded-md"
            type=""
          >
            Payment
          </button>
